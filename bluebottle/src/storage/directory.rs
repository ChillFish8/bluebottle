...
pub fn init_paths(base_path: Option<PathBuf>) -> Result<(), snafu::Whatever> {
    ...
    tracing::info!(
        ...
        cache = %paths.asset_cache_dir().display(),
...
impl DirectoryPaths {
    fn from_explicit_path(path: PathBuf) -> Self {
        ...
        Self {
            ...
            asset_cache_dir: cache_dir.join("assets"),
    ...
    fn ensure_created(&self) -> Result<(), snafu::Whatever> {
        ...
        std::fs::create_dir_all(&self.asset_cache_dir).with_whatever_context(|_| {
            format!("create directory {}", self.asset_cache_dir.display())
        })?;
    ...
    /// Returns the directory that should hold asset cache & temporary files.
    pub fn asset_cache_dir(&self) -> &Path {
        self.asset_cache_dir.as_path()
    }
